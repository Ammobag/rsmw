...
class Feed extends React.Component {
  ...
  componentDidMount() {
    ...
    setTimeout(
      function () {
        //Start the timer
        this.setState({ autoUpdate: false }); //After 1 second, set render to true
      }.bind(this),
      5000
    );
  }
        ...
        likedPosts.push(
          <div className="message">
            <div className="text-message">
              No one posts you liked. Press{" "}
              <i className={`far fa-heart`} style={{ color: "#604d92" }}></i>{" "}
              and try again
            </div>
          </div>
        );
    ...
    if (!elem.length)
      elem.push(
        <div className="message" key={-2}>
          <div className="text-message">
            No posts available. Please wait a few seconds
          </div>
        </div>
      );
    return elem;
        ...
        <div className="post-wrapper">
          {/* <div className="delete-button">
            <a
              ...
              onClick={this.props.args.deleteHandler}
            >
